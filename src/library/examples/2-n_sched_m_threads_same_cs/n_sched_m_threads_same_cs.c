...
int main() {
    ...
    int vars;
    struct completion_list* cs = completion_list_create();

    for(i=0; i<NUM_SCHED; i++){

        for(j=1; j<=NUM_WORKER; j++){
            id[j + i*(NUM_WORKER + 1)] = EnterUmsWorkingMode(foo1, &vars);
            completion_list_add(cs, id[j + i*(NUM_WORKER + 1)], j);
        }
        id[i*(NUM_WORKER + 1)] = EnterUmsSchedulingMode(cs, scheduler, &vars);
    }
    ...
    printf("Main exiting, final value of the counter:\n");
